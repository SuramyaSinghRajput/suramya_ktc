...
<div class="header">
  <h2>📦 <?php echo e($product->full_name); ?></h2>
</div>

<div class="filter-bar">
  <label for="warehouseFilter"><strong>Filter by Warehouse:</strong></label>
  <select id="warehouseFilter" data-product-id="<?php echo e($product->id); ?>">
...
<table id="lotTable" class="sortable-table" data-default-sort-index="1" data-default-sort-order="asc">
  ...
  <tbody id="lotTableBody">
    ...
    ?>
    <tr onclick="window.location='<?php echo e($link); ?>'" style="cursor: pointer;">
      ...
      <td>
        <?php if($imageFile): ?>
          <img src="<?php echo e(url('/storage/app/public/' . $imageFile->media_URL)); ?>" class="thumbnail" alt="Lot Image" onclick="zoomImage(event, '<?php echo e(url('/storage/app/public/' . $imageFile->media_URL)); ?>')">
        <?php else: ?>
          <span>No Image</span>
        <?php endif; ?>
      </td>
  ...
  <tfoot>
    <tr>
      ...
      <td><strong><?php echo e($lots->sum('quantity_bags')); ?></strong></td>
      <td><strong><?php echo e($lots->sum('remaining_quantity_bags_after_deduction')); ?></strong></td>
      <td colspan="2"></td>
    </tr>
  </tfoot>
</table>
...
<script>
  const baseUrl = "<?php echo e(url('/admin/warehouse/filter')); ?>";
  const baseDetailUrl = "<?php echo e(url('/admin/lots/details')); ?>";
    ...
    fetch(`${baseUrl}/${warehouseId}/${productId}`)
      ...
      .then(data => {
        ...
        data.lots.forEach(lot => {
          const permissionLot = <?php echo e($permission['lots'] ?? 0); ?>;
...
<?php echo $__env->make('admin.layout.default', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u791519547/domains/mrblogician.com/public_html/ktc/resources/views/admin/pages/product/product-lot.blade.php ENDPATH**/ ?>